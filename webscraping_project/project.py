...
def scrape_weather():
    print("[오늘의 날씨]")
    url = "https://search.naver.com/search.naver?where=nexearch&sm=top_hty&fbm=0&ie=utf8&query=%ED%9A%A1%EC%84%B1+%EB%82%A0%EC%94%A8"
    soup = create_soup(url)
    # 맑음, 어제보다 10˚ 낮아요
    cast = soup.find("p", attrs={"class":"cast_txt"}).get_text()
    # 현재 6℃ (최저 00˚ / 최고 00˚)
    curr_temp = soup.find("p", attrs={"class":"info_temperature"}).get_text().replace("도씨", "").strip()
